...
const Login = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (userCredential.role === "admin") {
        ...
        console.log("🔐 [Auth] Regular user logged in, redirecting to home");
        router.push('/');
      }
    } catch (err) {
  ...
  const handleGoogleLogin = async () => {
    try {
      ...
      // Redirect based on user role
      if (userCredential.role === "admin") {
        console.log("🔐 [Auth] Admin user logged in with Google, redirecting to admin dashboard");
        router.push('/adminUI');
      } else {
        console.log("🔐 [Auth] Regular user logged in with Google, redirecting to home");
        router.push('/');
      }
    } catch (err) {
          ...
          <form className="space-y-6" onSubmit={handleSubmit}>
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="text-sm">
                <a href="/auth/forgotPassword" className="font-medium text-green-600 hover:text-green-500">
